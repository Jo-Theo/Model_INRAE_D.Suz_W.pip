...
.eggs<-R6Class(classname = "eggs",
              ...
              public = list(t=NA,
                            ...
                            survie=function(temps_eclo,survie_egg){
                              ...
                              return(rbinom(n=2,size=nb_egg,survie_egg))
                            }
              ))
...
.larves<-R6Class(classname = "larves",
                 ...
                 public = list(t=NA,
                               ...
                               survie=function(temps_dev, survie_larves, taux_femelle = 0.5){
                                 ...
                                 emergents_ninf <- rbinom(self$t,self$ninf,proba)
                                 # liste des emmegents infecte
                                 emergents_inf <- rbinom(self$t,self$inf,proba)
                                 ...
                                 nb_adulte <- rbinom(n=2,size=nb_larves,survie_larves)
                                 # Tirage aleatoire pour la determination du sexe
                                 nb_fem <- rbinom(n=2,size=nb_adulte,taux_femelle)
...
.males<-R6Class(classname = "males",
               ...
               public = list(t=NA,
                             ...
                             survie=function(taux_survie){
                               return(rbinom(n=2, size=self$vivants(), prob = taux_survie))
                             },
...
.femelles<-R6Class(classname = "femelles",
                  ...
                  public = list(t=NA,
                                ...
                                # Idem aux males
                                survie=function(taux_survie){
                                  return(rbinom(n=4, size=self$vivants(), prob = taux_survie))
                                },
                                ...
                                ponte=function(fec_ninf,fec_inf,proba_ponte,transmissibilite,ic){
                                  ...
                                  nb_acces_ponte <- rbinom(n=4,self$vivants(),proba_ponte)
                                  nb_oeufs <- rep(NA,2)
                                  # tirage du nombre d'oeufs viables par categorie
                                  nb_oeufs[1] <- sum(rpois(n=4,lambda = fecondite*nb_acces_ponte*proba_viable*(1-transmissibilite)))
                                  nb_oeufs[2] <- sum(rpois(n=4,lambda = fecondite*nb_acces_ponte*proba_viable*transmissibilite))
                                  return(nb_oeufs)
                                }
                  ))
...
population<-R6Class(classname = 'population',
                    ...
                    public = list(t=NA,
                                  ...
                                  adultes=function(t=self$t,vector=F,all=F){
                                    if(vector){
                                      return(rowSums(self$adultes(all=T)[2:3]))
                                    }
                                    if(!all){
                                  ...
                                  step=function(vec_invasion = list(0,0,0), pourcentage = TRUE,param=self$parametres.fixes,fonct=self$parametres.fonctions){
                                    ...
                                    if (self$t <= length(vec_invasion[[1]])){
                                      new_male[2] <- new_male[2] + ifelse(pourcentage,round(vec_invasion[[1]][self$t]*sum(self$adultes())/100),round(vec_invasion[[1]][self$t]))
                                    }
                                    ...
                                    if (self$t <= length(vec_invasion[[3]])){
                                      new_vierge[2] <- new_vierge[2] + ifelse(pourcentage,round(vec_invasion[[3]][self$t]*sum(self$adultes())/100),round(vec_invasion[[3]][self$t]))
                                    }
                                    ...
                                    if (self$t <= length(vec_invasion[[2]])){
                                      new_femelle[4] <- new_femelle[4] + ifelse(pourcentage,round(vec_invasion[[2]][self$t]*sum(self$adultes())/100),round(vec_invasion[[2]][self$t]))
                                    }
                                  ...
                                  plot=function(pourcentage=FALSE,description=F,type="l",ech_log=T,main=NULL,
                                    ...
                                    if(pourcentage){
                                      ...
                                      lines(description$smooth_pop,col='red')
